<?php
	...
	$strQueryArt = "SELECT `ID_Art`, `Unidades` FROM `art_com` WHERE `ID_Com` = '$compra'";
	if ( !$queryArt = mysql_query( $strQueryArt, $db_resource ) ) sql_err("003");
	while ( $objRowArt = mysql_fetch_object( $queryArt, "art_com" ) ) {
		
		//OBTENER DATOS ARTICULO
		$articulo = $objRowArt->ID_Art;
		$unidades = $objRowArt->Unidades;
	...
	$strDelete = "DELETE FROM `compra` WHERE `ID` = '$compra'";

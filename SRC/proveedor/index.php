<?php 
	...
	$strQuery = "SELECT * FROM `proveedor` WHERE `Nombre` LIKE '%$nombre%' ORDER BY `ID` DESC LIMIT 10;";
	...
	while ( $objRow = mysql_fetch_object( $query, $seccion ) ) {
		...
		if ( $find ) {
			echo "<th><img style='cursor: pointer' src='./img/plus.gif' onclick=\"window.opener.seleccionar('".$objRow->Nombre."','slcProveedor')\" ></img></th>";
		}else{
			echo "<th><img style='cursor: pointer' src='./img/edit.gif' onclick=\"Pro.editPro('".$objRow->ID."')\" ></img></th>";
			echo "<th><img style='cursor: pointer' src='./img/delx.gif' onclick=\"Pro.delPro('".$objRow->ID."')\" ></img></th>";
		}
		echo "</tr>";
	}
	echo "</table>";
